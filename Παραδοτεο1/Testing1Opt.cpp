...
bool read_matrix_market_coo(const string &path, vector<MMEntry> &coords, bool &symmetric, u32 &max_index) {
    ...
    coords.clear();
    coords.reserve(nnz * (symmetric ? 2 : 1));
    max_index = 0;
...
void* degree_count_thread(void* arg){
    ...
    for(size_t i=0;i<td->n;i++)
        __sync_fetch_and_add(&(*td->deg)[i], local_deg[i]);
    return nullptr;
}
...
// Parallel compaction
void* compact_thread(void* arg){
    ThreadData* td = (ThreadData*)arg;
    u64 start = td->thread_id * td->n / td->num_threads;
    u64 end   = (td->thread_id+1) * td->n / td->num_threads;

    u64 offset = 0;
    for(u64 i=0;i<start;i++) offset += td->G->row_ptr[i+1] - td->G->row_ptr[i];

    for(u64 i=start;i<end;i++){
        u64 s = td->G->row_ptr[i], t = td->G->row_ptr[i+1];
        for(u64 p=s;p<t;p++) (*td->new_cols)[offset++] = td->G->col_idx[p];
        (*td->new_row_ptr)[i+1] = offset;
    }
    return nullptr;
}

// Build CSR (parallel)
CSR build_csr_undirected_pthread(u64 n, vector<MMEntry> &coords, int num_threads){
    vector<u64> deg(n,0);
    ...
    for(int i=0;i<num_threads;i++){
        td[i] = { (size_t)i, (size_t)num_threads, &coords, &deg, nullptr, nullptr, n, nullptr, nullptr };
    ...
    for(u64 i=0;i<n;i++) G.row_ptr[i+1] = G.row_ptr[i] + deg[i];

    G.col_idx.resize(G.row_ptr[n],0);
    vector<u64> cur(n);
    for(u64 i=0;i<n;i++) cur[i] = G.row_ptr[i];

    // Edge insertion
    for(int i=0;i<num_threads;i++){
        td[i] = { (size_t)i, (size_t)num_threads, &coords, nullptr, &cur, &G, n, nullptr, nullptr };
    ...
    for(int i=0;i<num_threads;i++){
        td[i] = { (size_t)i, (size_t)num_threads, nullptr, nullptr, nullptr, &G, n, nullptr, nullptr };
        pthread_create(&threads[i], nullptr, sort_dedup_thread, &td[i]);
    }
    for(int i=0;i<num_threads;i++) pthread_join(threads[i], nullptr);

    // Parallel compaction
    vector<u32> new_cols(G.col_idx.size());
    vector<u64> new_row_ptr(n+1,0);
    pthread_t cthreads[num_threads];
    for(int i=0;i<num_threads;i++){
        td[i] = { (size_t)i, (size_t)num_threads, nullptr, nullptr, nullptr, &G, n, &new_cols, &new_row_ptr };
        pthread_create(&cthreads[i], nullptr, compact_thread, &td[i]);
    }
    for(int i=0;i<num_threads;i++) pthread_join(cthreads[i], nullptr);
...
vector<u32> compute_connected_components(const CSR &G, size_t &num_iters){
    ...
    for(u64 i=0;i<n;i++) labels[i] = label_map[labels[i]];
    return labels;
}
...
int main(int argc, char **argv){
    ...
    fprintf(stderr,"Graph: n=%lu, stored edges (directed in CSR) = %zu\n", G.n, G.col_idx.size());
